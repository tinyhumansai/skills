...
from .types import EmailHostState

if TYPE_CHECKING:
  from collections.abc import Awaitable, Callable

log = logging.getLogger("skill.email.sync")
...
def _build_host_state() -> EmailHostState:
  s = get_state()
  return EmailHostState(
    connection_status=s.connection_status,
    is_initialized=s.is_initialized,
    account=s.account,
    total_unread=s.total_unread,
    folder_count=len(s.folders),
  )
